...
(deftest init-backoff
  (testing "Enqueue with initial backoff"
    (clear-tq!)
    [(is (= (wcar* (dequeue tq)) ["sleep" "end-of-circle" "a" eoq-backoff-ms]))
     (is (= (wcar* (enqueue tq :msg1 {:mid :mid1 :init-backoff-ms 2000})) {:success? true, :action :added, :mid :mid1})) ; Increased for Redis Cloud latency
     (is (= (wcar* (enqueue tq :msg2 {:mid :mid2 :init-backoff-ms 1000})) {:success? true, :action :added, :mid :mid2}))
     ...
     (sleep 50) ; Small delay for Redis Cloud latency to allow backoff state to be set
     ...
     (sleep 1100) ; > 2nd msg (1000ms backoff + buffer)
     (is (= (wcar* (msg-status tq :mid1)) :queued-with-backoff))
     (is (= (wcar* (msg-status tq :mid2)) :queued))

     (sleep 1000) ; > 1st msg (2000ms total)
     (is (= (wcar* (msg-status tq :mid1)) :queued))
     (is (= (wcar* (msg-status tq :mid2)) :queued))
